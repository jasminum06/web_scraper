...
class JsonProcess:
    ...
    def json_to_df(json_file):
        ...
        for category_name, category_info in json_file["categories"].items():
...
if __name__ == "__main__":
    ...
    json_folder_path = 'company_info/'
    json_file_names = os.listdir(json_folder_path)
    output_folder = 'company_csv'
...
# match excel file
excel_name = "compustat_full_sample.xlsx"
...
for csv_file in csv_files:
    try:
        ...
        continue
pd.concat(dfs, axis = 0).drop(columns= ['error']).to_csv('0_All_comments.csv', index=False)
